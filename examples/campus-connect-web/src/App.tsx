...
import {
  STUDENT, TIMETABLE, NOTES, EVENTS, NOTIFICATIONS, FEES, LECTURES,
  AppShellService, NetworkService, SyncService, LocaleService, SDK_LIBS,
} from './sdk';

// ─── Types ────────────────────────────────────────────────
type Page = 'dashboard' | 'timetable' | 'notes' | 'fees' | 'events' | 'notifications' | 'lectures' | 'idcard' | 'profile';
...
export function App() {
  ...
  return (
    <div className="app-layout">
      <Sidebar activePage={page} onNavigate={setPage} onLogout={() => setLoggedIn(false)} />
...
// ─── Sidebar ──────────────────────────────────────────────
function Sidebar({ activePage, onNavigate, onLogout }: { activePage: Page; onNavigate: (p: Page) => void; onLogout: () => void }) {
  const navItems: { section: string; items: { page: Page; icon: string; label: string }[] }[] = [
    {
      section: 'Main',
      items: [
    ...
    },
    {
      section: 'Account',
      items: [
        { page: 'profile', icon: '⚙️', label: 'Settings' },
      ],
    },
  ];
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="sidebar-footer">
        Powered by Rajeev SDK v0.2.1<br />
      ...
      <div className="login-card">
        ...
        {!otpSent ? (
          <>
            ...
            <button className="btn btn-primary" style={{ width: '100%', marginTop: 16, justifyContent: 'center' }} onClick={() => setOtpSent(true)}>
              Send OTP →
            </button>
            <div style={{ textAlign: 'center', margin: '16px 0', color: '#999', fontSize: 13 }}>or continue with</div>
            <button className="btn btn-secondary" style={{ width: '100%', justifyContent: 'center' }} onClick={onLogin}>
              🔐 Google Sign-In
            </button>
          </>
        ) : (
          <>
            <div className="form-group">
              ...
              <input className="form-input" placeholder="123456" value={otp} onChange={(e) => setOtp(e.target.value)} maxLength={6} />
            </div>
            <SdkBar libs={['auth', 'vault', 'notify']} />
            <button className="btn btn-primary" style={{ width: '100%', marginTop: 16, justifyContent: 'center' }} onClick={onLogin}>
              Verify & Sign In →
            </button>
          </>
        )}
  ...
  return (
    <div className="page">
      ...
      <div className="stats-grid">
        <div className="stat-card" onClick={() => onNavigate('timetable')} style={{ cursor: 'pointer' }}>
          <div className="stat-icon">📚</div>
          <div className="stat-value">{todayClasses.length}</div>
          <div className="stat-label">Classes Today</div>
          <div className="stat-sdk">@rajeev02/sync</div>
        </div>
        <div className="stat-card" onClick={() => onNavigate('fees')} style={{ cursor: 'pointer' }}>
          <div className="stat-icon">💰</div>
          <div className="stat-value">{LocaleService.formatCurrency(pendingFees)}</div>
      ...
      <div className="grid-2">
        ...
        <div className="card">
          ...
          <div className="table-wrap">
            <table>
              <thead><tr><th>Time</th><th>Subject</th><th>Room</th><th>Type</th></tr></thead>
              <tbody>
                {todayClasses.map((c) => (
                  <tr key={c.id}>
                    ...
                    <td>{c.subject}</td>
                    <td>{c.room}</td>
                    <td><span className={`badge ${c.type === 'Lab' ? 'badge-warning' : 'badge-primary'}`}>{c.type}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {[
              { icon: '📝', label: 'Scan Notes', sdk: 'camera + document', action: 'notes' as Page },
              { icon: '💳', label: 'Pay Fees', sdk: 'payments', action: 'fees' as Page },
              { icon: '🪪', label: 'Show ID', sdk: 'vault', action: 'idcard' as Page },
              { icon: '🎧', label: 'Lectures', sdk: 'media', action: 'lectures' as Page },
              { icon: '🎉', label: 'Events', sdk: 'deeplink', action: 'events' as Page },
              { icon: '📊', label: 'AI Quiz', sdk: 'edge-ai', action: 'notes' as Page },
            ].map((a) => (
              <div
                key={a.label}
                style={{ padding: 16, background: '#f5f5f5', borderRadius: 10, cursor: 'pointer', textAlign: 'center' }}
                onClick={() => onNavigate(a.action)}
              >
                <div style={{ fontSize: 28 }}>{a.icon}</div>
                <div style={{ fontWeight: 600, fontSize: 13, marginTop: 6 }}>{a.label}</div>
                <div style={{ fontSize: 9, color: '#999', marginTop: 4 }}>@rajeev02/{a.sdk}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="card">
        ...
        {NOTIFICATIONS.slice(0, 4).map((n) => (
          <div key={n.id} className={`notif-row ${!n.read ? 'unread' : ''}`}>
            <div className="notif-icon">{n.type === 'deadline' ? '⏰' : n.type === 'payment' ? '💰' : n.type === 'class' ? '📚' : n.type === 'event' ? '🎉' : '📢'}</div>
            <div style={{ flex: 1 }}>
      ...
      <SdkBar libs={['app-shell', 'auth', 'network', 'sync', 'ui', 'locale', 'notify']} />
    </div>
  );
}
  ...
  return (
    <div className="page">
      <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
        {days.map((d) => (
          <button key={d} className={`btn ${d === day ? 'btn-primary' : 'btn-secondary'} btn-sm`} onClick={() => setDay(d)}>
            {d}
          </button>
        ))}
      </div>
      <div className="card">
        ...
        <div className="table-wrap">
          <table>
            <thead><tr><th>Time</th><th>Code</th><th>Subject</th><th>Professor</th><th>Room</th><th>Type</th></tr></thead>
            <tbody>
              {filtered.length ? filtered.map((c) => (
                <tr key={c.id}>
                  ...
                  <td>{c.professor}</td>
                  <td>{c.room}</td>
                  <td><span className={`badge ${c.type === 'Lab' ? 'badge-warning' : c.type === 'Elective' ? 'badge-success' : 'badge-primary'}`}>{c.type}</span></td>
                </tr>
              )) : (
                <tr><td colSpan={6} style={{ textAlign: 'center', padding: 32, color: '#999' }}>No classes scheduled</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        {NOTES.map((note) => (
          <div key={note.id} className="note-card">
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
              ...
              <span className={`badge ${note.synced ? 'badge-success' : 'badge-warning'}`}>
                {note.synced ? '✅ Synced' : '🔄 Pending'}
              </span>
            </div>
            <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 8 }}>{note.title}</h3>
            <div style={{ fontSize: 12, color: '#999' }}>{note.pages} pages · {note.date}</div>
  ...
  return (
    <div className="page">
      <div className="stats-grid">
        <div className="stat-card">
          <div className="stat-icon">💰</div>
          <div className="stat-value">{LocaleService.formatCurrency(totalPending)}</div>
        ...
        <div className="stat-card">
          <div className="stat-icon">⏳</div>
          <div className="stat-value">{FEES.filter((f) => f.status === 'pending').length}</div>
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table>
            <thead><tr><th>Fee Item</th><th>Amount</th><th>Status</th><th>Date / Due</th><th>Action</th></tr></thead>
            <tbody>
              {FEES.map((f) => (
                <tr key={f.id}>
                  ...
                  <td>
                    <span className={`badge ${f.status === 'paid' ? 'badge-success' : f.status === 'pending' ? 'badge-warning' : 'badge-error'}`}>
                      {f.status === 'paid' ? '✅ Paid' : f.status === 'pending' ? '⏳ Pending' : '🔜 Upcoming'}
                    </span>
                  </td>
                  <td style={{ fontSize: 13 }}>{(f as any).date || (f as any).due}</td>
                  <td>
                    {f.status !== 'paid' && (
                      <button className="btn btn-primary btn-sm" onClick={() => alert(`Pay ${LocaleService.formatCurrency(f.amount)} via UPI\n\n@rajeev02/payments\n@rajeev02/vault`)}>
                        Pay Now
                      </button>
                    )}
                  </td>
          ...
          <div key={ev.id} className="event-card">
            ...
            <div className="event-body">
              ...
              <div style={{ margin: '8px 0', fontSize: 12 }}>
                ...
                {ev.fee > 0 && <span className="badge badge-warning" style={{ marginLeft: 6 }}>{LocaleService.formatCurrency(ev.fee)}</span>}
              </div>
...
function NotificationsPage() {
  ...
  return (
    <div className="page">
      <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
        {types.map((t) => (
          <button key={t} className={`btn ${t === filter ? 'btn-primary' : 'btn-secondary'} btn-sm`} onClick={() => setFilter(t)}>
            {t === 'all' ? '📋 All' : t === 'deadline' ? '⏰ Deadline' : t === 'payment' ? '💰 Payment' : t === 'class' ? '📚 Class' : t === 'event' ? '🎉 Event' : '📢 General'}
          </button>
        ))}
      </div>
      <div className="card">
    ...
    <div className="page">
      {playing !== null && (
        <div className="player-bar" style={{ marginBottom: 24 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, fontSize: 14 }}>{LECTURES.find((l) => l.id === playing)?.title}</div>
            <div style={{ fontSize: 11, opacity: 0.7 }}>{LECTURES.find((l) => l.id === playing)?.professor}</div>
            <div className="progress" style={{ marginTop: 8 }}>
              <div className="progress-fill" style={{ width: '35%' }} />
            </div>
          </div>
          <span style={{ cursor: 'pointer', fontSize: 22 }} onClick={() => setPlaying(null)}>⏹</span>
          <span className="sdk-tag" style={{ background: 'rgba(255,255,255,0.2)', color: '#fff' }}>@rajeev02/media</span>
        </div>
      )}
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table>
            <thead><tr><th>Title</th><th>Subject</th><th>Professor</th><th>Duration</th><th>Date</th><th>Action</th></tr></thead>
            <tbody>
              {LECTURES.map((l) => (
                <tr key={l.id}>
...
function IDCardPage() {
  ...
  return (
    <div className="page">
      ...
      <div className="id-card" style={{ cursor: 'pointer' }} onClick={() => setFlipped(!flipped)}>
        {!flipped ? (
          <div className="id-front">
            <div style={{ fontSize: 12, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 8 }}>
              Rajeev National University
            </div>
            <div style={{ fontSize: 48, margin: '16px 0' }}>{STUDENT.avatar}</div>
            <div style={{ fontSize: 20, fontWeight: 800 }}>{STUDENT.name}</div>
            <div style={{ fontSize: 14, opacity: 0.8, marginTop: 4 }}>{STUDENT.department}</div>
            <div style={{ display: 'flex', justifyContent: 'center', gap: 24, margin: '16px 0', fontSize: 13 }}>
              <div><strong>Roll No</strong><br />{STUDENT.rollNo}</div>
              <div><strong>Semester</strong><br />{STUDENT.semester}</div>
              <div><strong>Year</strong><br />{STUDENT.year}</div>
            </div>
            <div style={{ fontFamily: 'monospace', fontSize: 18, letterSpacing: 4, marginTop: 12 }}>
              ||||| {STUDENT.id} |||||
            </div>
          </div>
        ) : (
          <div className="id-back">
            <h3 style={{ fontSize: 14, marginBottom: 16 }}>Emergency Information</h3>
            ...
            <div style={{ marginTop: 16, padding: 12, background: '#e8f5e9', borderRadius: 8, fontSize: 11, color: '#2e7d32' }}>
    ...
    <div className="page">
          ...
          <div className="setting-row">
            <div className="setting-info">
              ...
              <div>
                ...
                <div className="setting-desc">@rajeev02/locale — {hindi ? 'हिंदी' : 'English'}</div>
              </div>
            </div>
            <div className={`toggle ${hindi ? 'active' : ''}`} onClick={() => setHindi(!hindi)} />
          </div>
          <div className="setting-row">
            <div className="setting-info">
              <span className="setting-icon">🌙</span>
              <div>
                ...
                <div className="setting-desc">@rajeev02/ui — {darkMode ? 'On' : 'Off'}</div>
              </div>
            </div>
          ...
          <div className="setting-row">
            <div className="setting-info">
              ...
              <div>
                ...
                <div className="setting-desc">@rajeev02/auth — Face ID / Touch ID</div>
              </div>
            </div>
        ...
        <div className="card">
          ...
          {Object.entries(AppShellService.featureFlags).map(([key, val]) => (
            <div className="setting-row" key={key}>
              <div className="setting-info">
                <div>
                  <div className="setting-label" style={{ fontFamily: 'monospace', fontSize: 13 }}>{key}</div>
                </div>
              </div>
              <span className={`badge ${val ? 'badge-success' : 'badge-error'}`}>{val ? 'ON' : 'OFF'}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="card" style={{ marginTop: 24 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {SDK_LIBS.map((lib) => (
            <span key={lib} className="badge badge-primary" style={{ fontSize: 12, padding: '6px 14px' }}>
              @rajeev02/{lib}
            </span>
          ))}
        </div>
...
function SdkBar({ libs }: { libs: string[] }) {
  return (
    <div className="lib-bar">
      {libs.map((lib) => (
        <span key={lib} className="sdk-tag">@rajeev02/{lib}</span>
      ))}
    </div>
  );
}
